...
public class Lienzo extends View {
    Paint p;
    int x, y;
    public Lienzo(Context c){ super(c);
    }
    protected void onDraw(Canvas c) {
        ...
        p.setTextAlign(Paint.Align.RIGHT); p.setTypeface(Typeface.SERIF); c.drawText("Eje X", x-5, y/2-10, p); p.setTextAlign(Paint.Align.CENTER); p.setTypeface(Typeface.DEFAULT_BOLD); c.drawText("Eje Y", x/2+30, 20, p);

        p.setColor(Color.argb(100, 200, 100, 100)); c.drawCircle(x/2-120, y/2+120, 100, p);
